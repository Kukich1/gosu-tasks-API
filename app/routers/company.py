...
async def show_posts(task_id: str):
    ...
    if da_ili_net:
        db = get_db()
        post_collection = db['posts']
        posts = await post_collection.find({'task': task_id},{'_id': 0, 'id': 1, 'name': 1, 'description': 1, 'member': 1, 'task': 1, 'created_at': 1,'time_completed': 1, 'type': 1, 'status': 1,'deadline': 1,'archive_deadline': 1,'action': 1}).to_list(length=None)
        return posts
    else:

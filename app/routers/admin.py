...
async def show_collection(username: str, current_user: str = Depends(get_current_user)):
    ...
    current_lst = user_posts + user_tasks
    return current_lst


@router.get("/show_completed_user_tasks&posts/{username}")
async def show_completed_taskpost(username: str, start: int = Query(default=0, ge=0), end: int = Query(default=2000000000, le=2000000000), current_user: str = Depends(get_current_user)):
    db = get_db()
    posts_collection = db['posts']
    task_collection = db['tasks']
    project_collection = db['projects']
    
    user_posts = await posts_collection.find({"time_completed":{"$gte": start, "$lte": end}, "member": username, 'status': 'completed'}, {'_id': 0, 'id': 1, 'name': 1, 'description': 1, 'task': 1,'deadline': 1, 'created_at': 1, 'type': 1, 'status': 1, 'time_completed': 1, 'action': 1, 'member': 1}).to_list(length=None)
    user_tasks = await task_collection.find({"time_completed":{"$gte": start, "$lte": end}, "members": username, 'status': 'completed'}, {'_id': 0, 'id': 1, 'name': 1, 'description': 1, 'project': 1, 'deadline': 1, 'created_at': 1, 'comments': 1, 'type': 1, 'status': 1, 'members': 1,'time_completed': 1}).to_list(length=None)
    ...
    for user_task in user_tasks:
        ...
        project_name = await project_collection.find_one({'id': project_id}, {'name': 1})
        user_task["project_name"] = project_name['name'] if project_name else "Неизвестный проект"
        user_task["project"] = str(project_id)
